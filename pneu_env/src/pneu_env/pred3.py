...
class PneuPred:
    def __init__(
        ...
        lib_path = env_pkg_path / "src/pneu_env/lib3/libpneumatic_simulator_pred.so"
        if not lib_path.is_file():
            raise FileNotFoundError(f"Could not find lib3 predictor library: {lib_path}")

        self.lib = CDLL(str(lib_path))
        print(f"[ INFO] Loaded pneumatic predictor library from: {lib_path}")
        ...
        print(f"[ INFO] Pneumatic Predictor ==> Delay: {delay}")
    ...
    def observe(
        ...
        if ctrl.shape != (6,):
            raise ValueError(f"pred3 expects 6 control inputs, got shape {ctrl.shape}")
        if goal.shape != (2,):
            raise ValueError(f"pred3 expects 2 actuator reference inputs, got shape {goal.shape}")
